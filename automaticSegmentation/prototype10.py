# -*- coding: utf-8 -*-
"""
Created on Fri Dec  5 11:53:49 2025
...
# -*- coding: utf-8 -*-
"""
U-Net for lung tumour segmentation – PROTOTYPE 10
 ...
 - Image preprocessing:
     * Normalisation + CLAHE (equalize_adapthist) to improve contrast
 - Mask post-processing (GT and predictions):
     * Largest connected component via regionprops
 - Saves:
     * X_val_modelo10.npy
     * Y_val_modelo10_bce_dice_focal.npy
     * preds_val_modelo10_bce_dice_focal_probs.npy
     * preds_val_modelo10_bce_dice_focal_bestthr.npy
     * preds_val_modelo10_bce_dice_focal_bestthr_pp.npy
 - Generates PNGs in folder:
     * resultados_val_modelo10_png
   Each PNG shows (in a single figure):
     * Preprocessed original image
     * Clean ground truth (GT)
...
# (Used for GT and predictions)
# ------------------------------------------------------------------
def postprocess_mask(mask_bin):
    """
    mask_bin: array (H, W) with 0/1 (float or uint8)
    Returns a mask with only the largest connected component.
    If there are no positive pixels, returns the original mask.
    """
...
# ------------------------------------------------------------------
# INTENSITY / CONTRAST PREPROCESSING
# ------------------------------------------------------------------
def preprocess_image(img):
    """
    img: (H, W) or (H, W, 1), values in [0, 255] or [0, 1]
    - Normalises to [0,1]
    - Applies CLAHE (equalize_adapthist) to improve contrast
    - Returns (H, W, 1) in [0,1]
    """
    ...
    img_eq = exposure.equalize_adapthist(img2d, clip_limit=0.01)
...
def load_dataset(images_dirs, masks_dirs, img_height, img_width):
    """
    images_dirs, masks_dirs: lists of folders (to combine train + train_aug)
    Returns X, Y normalised and binarised.
    Applies:
     - Intensity/contrast preprocessing to images
     - GT cleaning with postprocess_mask (largest component)
    """
    # 1) Collect paths
    image_paths_raw = []
    mask_paths_raw  = []
    ...
    print("Loading, resizing and preprocessing images and masks...")
    ...
    for i, (img_path, mask_path) in enumerate(zip(image_paths, mask_paths)):
        ...
        # Clean GT with largest connected component
        mask_clean = postprocess_mask(mask)

        mask_clean = mask_clean.astype(np.float32)
        mask_clean = np.expand_dims(mask_clean, axis=-1)
...
np.save("X_val_modelo10.npy", X_val)

# ------------------------------------------------------------------
# INFO ABOUT SLICES WITH TUMOUR
# ------------------------------------------------------------------
...
checkpointer = tf.keras.callbacks.ModelCheckpoint(
    'unet_lung_256_gray_modelo10_bce_dice_focal.h5',
    verbose=1,
    save_best_only=True,
    monitor='val_loss'
)
...
tensorboard_cb = tf.keras.callbacks.TensorBoard(log_dir='logs_modelo10_bce_dice_focal')
...
# Ground truth
np.save("Y_VAL_MODEL10.npy", Y_val)

# Probabilities (sigmoid output)
np.save("PREDS_VAL_MODEL10.npy", preds_val_probs)

# Prediction with threshold 0.90
np.save("PREDS_VAL_MODEL10_BEST.npy", preds_val_bestthr)

# Prediction with threshold + post-processing
np.save("PREDS_VAL_MODEL10_BEST_PP.npy", preds_val_bestthr_pp)

print("Saved for metrics (PROTOTYPE 10):")
print("- Y_VAL_MODEL10.npy")
print("- PREDS_VAL_MODEL10.npy               (sigmoid probabilities 0-1)")
print("- PREDS_VAL_MODEL10_BEST.npy          (threshold 0.90)")
print("- PREDS_VAL_MODEL10_BEST_PP.npy       (threshold + post-processing)")
...
# ------------------------------------------------------------------
OUT_DIR = "resultados_val_modelo10_png"
...
for i in range(num_ejemplos):
    ...
    axs[0].set_title("Image (preprocessed)")
    axs[0].axis("off")

    axs[1].imshow(gt, cmap="gray")
    axs[1].set_title("GT (clean)")
    axs[1].axis("off")

    axs[2].imshow(pred, cmap="gray")
    axs[2].set_title("Prediction (postproc)")
    axs[2].axis("off")

    fig.suptitle(f"Validation - slice {i:03d} - PROTOTYPE 10", fontsize=10)
    fig.tight_layout()

    fig.savefig(os.path.join(OUT_DIR, f"val_{i:03d}_modelo10.png"), dpi=150)
    plt.close(fig)
